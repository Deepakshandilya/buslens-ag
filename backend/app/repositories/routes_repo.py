...
def find_route_matches(db: Session, from_stop: str, to_stop: str,limit: int = 20) -> list[dict]:
    ...
    try:
        ...
        raise ValueError("Database error while finding route matches") from e
...
def get_stops_between(db: Session, route_id: int, from_seq: int, to_seq: int) -> list[str]:
    ...
    try:
        ...
        raise ValueError("Database error while getting stops between") from e
...
def get_route_detail(db: Session, route_number: str, direction: str) -> dict | None:
    ...
    try:
        ...
        raise ValueError(f"Database error while getting route detail for {route_number}") from e
...
def get_routes_for_stop(db: Session, stop_id: int) -> dict | None:
    try:
        ...
        raise ValueError(f"Database error while getting routes for stop {stop_id}") from e